...
import { deleteServiceAction, toggleServiceStatusAction } from '../../actions';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function ActionButtons({ item }: { item: any }) {
  const handleDelete = async () => {
    if (confirm('Apakah Anda yakin ingin menghapus layanan ini?')) {
      const result = await deleteServiceAction(item.id);
  ...
  const handleToggle = async () => {
    const result = await toggleServiceStatusAction(item.id, item.isActive);
    if (result.success) toast.success(result.message);
    else toast.error(result.error);
  };
